...
public:
    ...
    void start()
    {
        ...
        for (auto triangle : triangles)
        {
            ...
            if (mid_point.x > image_cols)
            {
                mid_point.x = image_cols - 1;
            }
            if (mid_point.y < 0)
            {
                mid_point.y = 0;
            }
            if (mid_point.y > image_rows)
            {
                mid_point.y = image_rows - 1;
            }
            if (mid_point.x < 0 || mid_point.x > image_cols)
            {
                std::cout << "mid_point.x X failed" << std::endl;
            }
            if (mid_point.y < 0 || mid_point.y > image_rows)
            {
                std::cout << "mid_point.y Y failed" << std::endl;
            }
            ...
            if (temp_circum_point[0] < 0)
            {
                temp_circum_point[0] = 0;
            }
            if (temp_circum_point[0] > image_cols)
            {
                temp_circum_point[0] = image_cols;
            }
            if (temp_circum_point[1] < 0)
            {
                temp_circum_point[1] = 0;
            }
            if (temp_circum_point[1] > image_rows)
            {
                temp_circum_point[1] = image_rows;
            }
            if (temp_circum_point[2] < 0)
            {
                temp_circum_point[2] = 0;
            }
            if (temp_circum_point[2] > image_cols)
            {
                temp_circum_point[2] = image_cols;
            }
            if (temp_circum_point[3] < 0)
            {
                temp_circum_point[3] = 0;
            }
            if (temp_circum_point[3] > image_rows)
            {
                temp_circum_point[3] = image_rows;
            }

            if (temp_circum_point[0] < 0 || temp_circum_point[0] > image_cols)
            {
                std::cout << "min_x X failed" << std::endl;
            }
            if (temp_circum_point[1] < 0 || temp_circum_point[1] > image_rows)
            {
                std::cout << "min_y Y failed" << std::endl;
            }
            if (temp_circum_point[2] < 0 || temp_circum_point[2] > image_cols)
            {
                std::cout << "max_x X failed" << std::endl;
            }
            if (temp_circum_point[3] < 0 || temp_circum_point[3] > image_rows)
            {
                std::cout << "max_y Y failed" << std::endl;
            }
    ...
    void save()
    {
        ...
        cv::imwrite("C:/Users/xiaoshuyui/Desktop/结果图.jpg", result_image);
    }
...
private:
    ...
    */
    ...
    std::vector<int> circumRect(TrianglePoints &tri)
    {
        ...
        int min_x = ((tri.p1.x < tri.p2.x) ? (tri.p1.x < tri.p3.x ? tri.p1.x : tri.p3.x) : (tri.p2.x < tri.p3.x) ? tri.p2.x
                                                                                                                 : tri.p3.x);
        int min_y = ((tri.p1.y < tri.p2.y) ? (tri.p1.y < tri.p3.y ? tri.p1.y : tri.p3.y) : (tri.p2.y < tri.p3.y) ? tri.p2.y
                                                                                                                 : tri.p3.y);
        int max_x = ((tri.p1.x > tri.p2.x) ? (tri.p1.x > tri.p3.x ? tri.p1.x : tri.p3.x) : (tri.p2.x > tri.p3.x) ? tri.p2.x
                                                                                                                 : tri.p3.x);
        int max_y = ((tri.p1.y > tri.p2.y) ? (tri.p1.y > tri.p3.y ? tri.p1.y : tri.p3.y) : (tri.p2.y > tri.p3.y) ? tri.p2.y
                                                                                                                 : tri.p3.y);
